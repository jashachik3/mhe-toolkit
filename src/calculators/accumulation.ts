...
export interface AccumulationResult {
  totalLengthFt: string;
  bufferTimeSec: string;
  bufferTimeMin: string;
  packageCapacity: number;
}

// Pure math for the Accumulation Buffer / Time calculator.
export function calcAccumulation({ speed, zoneLength, zoneCount }: AccumulationInput): AccumulationResult | null {
