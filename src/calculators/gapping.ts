...
export interface GapResultInput {
  parcelLength: string;
  inputSpeed: string;
  gapperSpeeds: string[];
}
...
export function calcGapResult({ parcelLength, inputSpeed, gapperSpeeds }: GapResultInput): GapResult | null {
...
export interface MonteCarloGapInput {
  minParcelLength: string;
  parcelLength: string;
  maxParcelLength: string;
  inputSpeed: string;
  gapperSpeeds: string[];
}

export interface MonteCarloGapResult {
  bins: number[];
  counts: number[];
  mean: number;
  stdDev: number;
...
export function calcMonteCarloGap({
  minParcelLength,
  parcelLength,
  maxParcelLength,
  inputSpeed,
  gapperSpeeds,
}: MonteCarloGapInput): MonteCarloGapResult | null {
  ...
  const gaps = new Array<number>(SAMPLE_COUNT);
  let sum = 0;
