...
export function GapCalc({ setView }: ScreenProps) {
  ...
  const [gapperSpeeds, setGapperSpeeds] = usePersistentJSON<string[]>("gap.gapperSpeeds", ["75", "95", "120"]);
  ...
  const setGapperSpeedAt = (i: number, val: string) => {
    setGapperSpeeds((prev) => {
      const next = [...prev];
      next[i] = val;
      return next;
  ...
  const monteCarlo = useMemo(() => {
    ...
    if (!raw) return null;
    const cn = (n: number) => convertNumber(n, "length_in", system);
    return {
      ...raw,
