...
import { convertValue, type UnitKind, type UnitSystem } from "./units";
...
// change with the unit system. `kind: UnitKind` here is exactly where a
// typo'd kind string (e.g. "lenght_in") now fails to compile, instead of
// silently no-op'ing at runtime like it would pre-TypeScript.
const SCALAR_FIELDS: Record<string, ScalarFieldDef> = {
...
export const ARRAY_FIELDS: Record<string, ArrayFieldDef> = {
...
type FieldSnapshot = Record<string, string | string[]>;
...
export function computeConvertedFields(
...
export function switchUnitSystem(newSystem: UnitSystem): void {
  const current = (window.localStorage.getItem("mhe-toolkit:unitSystem") || "imperial") as UnitSystem;
  ...
  try {
    ...
    for (const key of Object.keys(ARRAY_FIELDS)) {
      const raw = window.localStorage.getItem(`mhe-toolkit:${key}`);
      if (raw !== null) snapshot[key] = JSON.parse(raw) as string[];
    }

    const converted = computeConvertedFields(snapshot, current, newSystem);
    for (const [key, value] of Object.entries(converted)) {
      const fullKey = `mhe-toolkit:${key}`;
      window.localStorage.setItem(fullKey, key in ARRAY_FIELDS ? JSON.stringify(value) : (value as string));
    }
